...
public abstract class BaseEntity<ID extends Serializable> extends PersistableEntity<ID> implements DefaultAuditable<String, ID> {
    ...
    @MetaData(value = "Optimistic locking version")
    @Version
    ...
    @MetaData(value = "Record the data packet", comments = "Identification data sources , such as different plants , structure of the code , APP names, etc.")
    @Column(length = 100)
    @JsonView(JsonViews.Admin.class)
