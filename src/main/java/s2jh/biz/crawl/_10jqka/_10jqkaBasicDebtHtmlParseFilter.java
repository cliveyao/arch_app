...
public class _10jqkaBasicDebtHtmlParseFilter extends AbstractHtmlParseFilter {
    ...
    private final static Set<String> whiteList = Sets.newHashSet(new String[] { "Total shareholders' equity" });
    ...
    public DBObject filterInternal(String url, WebPage webPage, DBObject parsedDBObject) throws Exception {
     ...
     // If an error message back , directly or null
        if (content.contains("<img src=\"http://i.thsi.cn/images/basic/stock/sub_page_bg.png\" alt=\"prompt\" />")) {
            return null;
        }
            ...
            for (int j = 0; j < dates.length; j++) {
                ...
                if (dateWhiteList.contains(date)) {
                    for (int i = 1; i < map.get("report").size(); i++) {
                     ...
                     // Vegetables stored in the whitelist
