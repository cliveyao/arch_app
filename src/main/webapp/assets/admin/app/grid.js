	...
	return {
			...
			$.jgrid
					.extend({
						...
						getOnlyOneSelectedItem : function(e) {
							var g = $(this);
							var d = [];
							var f = jQuery(g).jqGrid("getGridParam",
									"selarrrow");
							if (f.length > 0) {
								for (var b = 0; b < f.length; b++) {
									var c = $(
											"#jqg_" + jQuery(g).attr("id")
													+ "_" + f[b]).is(
											":disabled");
									if (!c) {
										d.push(f[b])
								...
								var h = jQuery(g).jqGrid("getGridParam",
										"selrow");
								if (h) {
									d.push(h)
								}
		...
		initGrid : function(X, G) {
			...
			var N = $
					.extend(
							...
							$.jgrid.defaults,
							{
								...
								inlineNav : {
									add : y.editurl || d == "items" ? true
											: false,
									edit : y.editurl || d == "items" ? true
											: false,
									del : y.delurl || d == "items" ? true
											: false,
										...
										if (N.footerLocalDataColumn) {
											...
											$
													.each(
															N.colModel,
															function(aA, az) {
																if (az.footersum != undefined
																		&& az.footersum == false) {
																	return
																}
																if (az.formatter == "integer"
																		|| az.formatter == "currency"
																		|| az.formatter == "number") {
																	var aB = ak
																			.jqGrid(
																					"sumColumn",
			...
			$
					.each(
							...
							function(ay, ax) {
								...
								if (ax.stype == "date" || ax.sorttype == "date"
										|| ax.formatter == "date"
										|| ax.formatter == "timestamp"
										|| ax.formatter == "shortTimestamp") {
																	...
																	.daterangepicker(
																			$
																					.extend(
																							...
																							ax.searchoptions.daterangepicker),
																			function(
																					aH,
																					aG) {
																				$(
														...
														dataInit : function(aF) {
															...
															if (ax.editoptions.spellto) {
																aE
																		.change(function() {
																			var aH = {};
																			aH[ax.editoptions.spellto] = Pinyin
																					.getCamelChars($
																							.trim(aE
																									.val()));
																			ak
								...
								if (ax.formatter == "select") {
									...
									if (typeof ax.searchoptions.value === "function") {
										ax.searchoptions.value = ax.searchoptions.value
												.call(ak)
									}
									if (ax.searchoptions.valueJsonString) {
										...
										var aD = $
												.parseJSON(ax.searchoptions.valueJsonString);
										for ( var aC in aD) {
								...
								if (ax.hasOwnProperty("searchoptions")) {
									...
									if (aA.hasOwnProperty("defaultValue")
											&& aA.defaultValue != "") {
										var aB = ax.index;
										if (aB == undefined) {
											aB = ax.name
								...
								if (ax.searchoptions.sopt == undefined) {
									ax.searchoptions.sopt = [ "cn", "bw", "bn",
											"eq", "ne", "nc", "ew", "en" ]
								}
								if (ax.index == undefined) {
									if (ax.formatter == "date"
											|| ax.formatter == "timestamp") {
										...
										if (ax.formatter == "checkbox") {
											...
											if (ax.formatter == "integer"
													|| ax.formatter == "currency"
													|| ax.formatter == "number"
													|| ax.formatter == "percentage") {
			...
			if (N.pager || N.toppager) {
				...
				if (N.pager
						&& (N.inlineNav.add || N.inlineNav.edit || N.inlineNav.del)
						&& N.inlineNav != false) {
		...
		initRecursiveSubGrid : function(f, c, e, h) {
			...
			d.url = Util.AddOrReplaceUrlParameter(d.url, "search['EQ_" + e
					+ "']", c);
			if (d.fullediturl) {
				d.fullediturl = Util.AddOrReplaceUrlParameter(d.fullediturl, e,
						c)
			}
